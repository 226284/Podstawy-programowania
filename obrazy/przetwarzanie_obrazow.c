...
W kolejnych testach, czy funkcje edytuj±ce obraz dzia³aj± prawid³owo. 
...
lega³o wiêc na wizualnym porównaniu przetworzonych obrazów z mojego programu, z tymi 
w pliku pdf.
...
W tym przypadku musia³em pos³u¿yæ siê obrazem "Lena.pgm", gdy¿ plik "kubus.pgm" zawiera³
...
szaro¶ci obrazów. Jednak aby sprawdziæ czy rozci±ganie przebieg³o prawid³owo wczyta³em
...
void progowanie(int obraz_pgm[][MAX], int *wymx, int *wymy, int *szarosci, int PROG){
  ...
  for(i=0; i<*wymy; i++){
	...
	obraz_pgm[i][j]=MAX_KOL;  /* L(x,y)  je¿eli L(x,y)> PROG */ 
				  /* (warunek "sam siê wykona")  */
    }
  }
}
...
void korekcja(int obraz_pgm[][MAX], int *wymx, int *wymy, float GAMMA){
  ...
  for(i=0; i<*wymy; i++){
    ...
    }                             /*U¿ycie liczb typu float, aby zapobiec
				    utracie danych                       */
 }
}
  ...
  for(i=1; i<*wymy-1; i++){
    for(j=0; j<*wymx; j++){
      ...
      obraz_pgm[i][j]=(1.0f/3.0f)*(obraz_pgm[i-1][j] + obraz_pgm[i][j] + obraz_pgm[i+1][j]);        
                                  /*Tu równie¿ u¿ycie zmiennych typu float */
...
void histogram(int obraz_pgm[][MAX], int *wymx, int *wymy){
  ...
  int max_wart=0, min_wart=255;           /*Mia³o byæ na charach, ale co¶ nie chcia³o dzia³aæ*/
...
int main() {
  ...
  printf("\n"
	 "\nUWAGA! MOZNA WYKONAC KILKA OPERACJI NA JEDNYM OBRAZIE,"
	 "\nNALEZY JEDNAK PAMIETAC, ZE KOLEJNOSC ICH WYKONYWANIA MA ZNACZENIE"
	 "\n");
  ...
  do{
    ...
    switch(opcja)
    {
	  ...
	  printf("\nPodaj wspolczynnik gamma [w formacie (cyfra.cyfra)]: \n");
	  scanf("%f",&gamma);
